...
/**
 * Holds the authenticated tenant for the current request. Requests run on a single
 * (virtual) thread, so a ThreadLocal is sufficient; {@link ApiKeyFilter} sets and
 * clears it. Every downstream operation takes its tenant scope from here —
 * there is no code path without a tenant (Constitution I).
 */
