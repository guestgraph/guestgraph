...
/**
 * Exact matching on shared strong identifiers; every decision has confidence 1.0.
 * An identifier shared by more records than the tenant's review threshold is a
 * classic shared-address signal (family, agency, front desk) — those candidates
 * are parked for human review instead of merged (research R9).
 */
public class DeterministicMatcher implements ResolutionStrategy {
    ...
    @Override
    public List<MatchDecision> score(SourceRecord record, List<MatchCandidate> candidates, int reviewThreshold) {
