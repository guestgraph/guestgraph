...
/**
 * Everything the resolution engine needs from storage. The engine itself is
 * storage-agnostic so the table-driven scenario tests run on a pure in-memory
 * implementation; production wires a Postgres-backed adapter.
 */
public interface GraphPort {
    ...
    /** Distinct source records in the tenant carrying this identifier (including the one being resolved). */
    ...
    void replaceGuestIdentifiers(UUID tenantId, UUID guestId, Collection<NormalizedIdentifier> identifiers);
