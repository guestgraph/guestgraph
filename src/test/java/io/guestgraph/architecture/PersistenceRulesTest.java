 ...
 * reviewable in every query, and repository scaffolding that bypasses it (CrudRepository&#38;co) is
 * banned.
 */
class PersistenceRulesTest {
  ...
  void everyRepositoryMethodIsTenantScoped() {
    methods()
        ...
        .resideInAPackage("io.guestgraph.persistence.repo")
        .should(
            new ArchCondition<>(
                "take a tenantId parameter or be @TenantAgnostic with a justification") {
              @Override
              public void check(JavaMethod method, ConditionEvents events) {
                if (method.getOwner().isAnnotation()
                    || method.isAnnotatedWith(TenantAgnostic.class)) {
                  return;
                }
                boolean hasTenantId =
