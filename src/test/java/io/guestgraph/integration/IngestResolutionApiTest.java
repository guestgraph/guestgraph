...
class IngestResolutionApiTest extends PostgresIntegrationTest {
    ...
    void newEmailCreatesGuest() {
        JsonNode result = ingestOne(TENANT_A_KEY, """
    ...
    void sameEmailDifferentCaseAttachesToExistingGuest() {
        JsonNode first = ingestOne(TENANT_A_KEY, """
                {"sourceSystem":"opera-pms","externalKey":"r-1","payload":{"email":"anna@example.com"}}
                """);
        JsonNode second = ingestOne(TENANT_A_KEY, """
    ...
    void recordSharingIdentifiersWithTwoGuestsMergesThem() {
        JsonNode a = ingestOne(TENANT_A_KEY, """
                {"sourceSystem":"opera-pms","externalKey":"a","payload":{"email":"a@example.com"}}
                """);
        JsonNode b = ingestOne(TENANT_A_KEY, """
        ...
        JsonNode c = ingestOne(TENANT_A_KEY, """
        ...
        Integer guests = jdbc.sql("SELECT count(*) FROM guest WHERE tenant_id = :t").param("t", TENANT_A)
                .query(Integer.class).single();
    ...
    void malformedButParseableRecordIsStoredFlaggedNeverDropped() {
        ...
        assertThat(result.get("guestId").asString()).isNotBlank();
        JsonNode stored = jdbc.sql("SELECT needs_review, needs_review_reasons::text AS reasons FROM source_record WHERE external_key = 'bad-1'")
                .query((rs, i) -> json("{\"needsReview\":" + rs.getBoolean("needs_review")
    ...
    void unparseableBodyIsRejectedAndNothingStored() {
        ResponseEntity<String> response = api(TENANT_A_KEY).post().uri("/api/v1/records")
    ...
    void concurrentIngestSharingAnIdentifierYieldsOneConsistentGuest() throws Exception {
        ...
        Integer guests = jdbc.sql("SELECT count(*) FROM guest WHERE tenant_id = :t").param("t", TENANT_A)
                .query(Integer.class).single();
        assertThat(guests).isEqualTo(1);
        Integer links = jdbc.sql("SELECT count(*) FROM resolution_link WHERE tenant_id = :t").param("t", TENANT_A)
                .query(Integer.class).single();
    ...
    private void registerSource(String apiKey, String code) {
        api(apiKey).post().uri("/api/v1/source-systems")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    private JsonNode ingestOne(String apiKey, String body) {
        ResponseEntity<String> response = api(apiKey).post().uri("/api/v1/records")
                .contentType(MediaType.APPLICATION_JSON)
                .body(body)
                .retrieve().toEntity(String.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
